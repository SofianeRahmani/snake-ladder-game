...
T ListeCirculaire<T>::element(int pos) const
{
   ...
   // La position
   if (pos < 1 || pos > taille())
      throw std::range_error("Liste Circualire Element:Position de l'element est erronee");
 ...
 */
    ...
    void ListeCirculaire<T>::enleverEl(const T& x) {
        if (dernier == 0) {
            throw std::logic_error("EnleverEl: L'élément dont on veut enlever n'est pas présent dans la liste (liste vide!)");
        }
        ...
        if (courant->element != x) {
            throw std::logic_error("EnleverEl: L'élément à enlever n'est pas présent dans la liste");
        }


        if (courant == dernier) {
            if (dernier == dernier->suivant) {
                delete dernier;
                dernier = nullptr;
            } else {
